    ...
    >
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
        ...
        >
          ...
          <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-background/80 border border-border/50 text-[10px] uppercase tracking-wider text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity">
            Change
          </div>
        </motion.button>
        ...
        <div className="flex-1 text-center md:text-left">
          <p className="text-[10px] font-ui tracking-[0.4em] text-primary/80 uppercase mb-2">Sanctuary Identity</p>
          <h2 className="text-3xl md:text-4xl font-display gold-text">{nickname || 'Anonymous'}</h2>
          <p className="text-xs text-muted-foreground mt-1 capitalize">
            {avatar.label} · {identityMode ?? 'guest'}
            {email && <> · {email}</>}
          </p>

          <div className="mt-6 grid grid-cols-3 gap-3 max-w-md mx-auto md:mx-0">
            <Stat label="Sessions" value={sessionCount} />
            <Stat label="Mood" value={dominantEmotion ?? '—'} capitalize />
