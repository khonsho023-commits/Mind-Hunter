...
export default function EditProfileForm() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-2xl p-6">
      <h3 className="text-xs font-ui tracking-[0.25em] text-muted-foreground uppercase mb-5">Editable Information</h3>
      <div className="space-y-4">
        <Field label="Display Name" value={nickname} onChange={setNickname} error={errors.nickname} />
        <div className="grid grid-cols-2 gap-4">
          <Field label="Age" value={age} onChange={setAge} type="number" error={errors.age} />
          <Field label="Gender" value={gender} onChange={setGender} error={errors.gender} />
        </div>
        {profile?.email && <Field label="Email" value={profile.email} disabled />}
      </div>
