...
import { Phone, Send, ShieldAlert, ArrowLeft } from 'lucide-react';
...
const HOTLINES = [
  { name: '988 Suicide & Crisis Lifeline', detail: 'Call or text 988 (US)' },
  { name: 'Crisis Text Line', detail: 'Text HOME to 741741' },
  { name: 'International Helplines', detail: 'findahelpline.com' },
];

const SUPPORTIVE_REPLIES = [
  "I hear you, and I'm here. Take a slow breath with me — you don't have to carry this alone right now.",
  "Thank you for telling me. What you're feeling makes sense. Are you safe in this moment?",
  "I'm staying right here with you. Can you describe one thing in the room around you?",
];

export default function EmergencyChat() {
  const { setStage } = useApp();
  const [messages, setMessages] = useState<Msg[]>([
    { role: 'support', content: "You're in a safe space. I'm here to listen — there's no rush. What's happening right now?" },
  ]);
    ...
    <PageShell>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass-strong rounded-2xl p-6 mb-6 border border-destructive/30">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h2 className="text-xl font-display text-foreground mb-1">Emergency Support</h2>
            <p className="text-sm text-muted-foreground">
              If you're in immediate danger, please reach out to a real human via the hotlines below. This space is here to help you breathe and stay grounded.
            </p>
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass rounded-2xl flex flex-col h-[60vh]">
            ...
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-md rounded-2xl px-4 py-3 text-sm font-body leading-relaxed ${
                  m.role === 'user' ? 'bg-primary/20 border border-primary/30' : 'glass border-l-2 border-l-destructive/40'
                }`}>
                  {m.content}
                </div>
              </div>
          ...
          <div className="p-4 border-t border-border/30 flex gap-2">
            ...
            />
            <button onClick={send} className="sentinel-btn px-4">
        ...
        <div className="space-y-3">
          <h3 className="text-xs font-ui tracking-[0.25em] text-muted-foreground uppercase">Immediate Help</h3>
          {HOTLINES.map(h => (
