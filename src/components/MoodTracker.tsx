...
const MoodTracker = forwardRef<HTMLDivElement, MoodTrackerProps>(({ emotionLog, elapsed, breathingUsed }, ref) => {
  ...
  const trendLabel = avgSentiment > 0.2 ? 'Improving' : avgSentiment < -0.2 ? 'Challenging' : 'Steady';
  const trendEmoji = avgSentiment > 0.2 ? '📈' : avgSentiment < -0.2 ? '📉' : '➡️';

  return (
    <div ref={ref} className="space-y-4">
      <div>
        <p className="text-xs font-ui text-muted-foreground mb-1">Session Duration</p>
        <p className="text-lg font-display text-foreground">{formatTime(elapsed)}</p>
      </div>

      <div>
        <p className="text-xs font-ui text-muted-foreground mb-1">Mood Trend</p>
      ...
      {emotionLog.length > 0 && (
        <div>
          <p className="text-xs font-ui text-muted-foreground mb-2">Intensity Over Time</p>
      ...
      <div>
        <p className="text-xs font-ui text-muted-foreground mb-1">Average Intensity</p>
      ...
      {breathingUsed > 0 && (
        <div>
          <p className="text-xs font-ui text-muted-foreground mb-1">Breathing Exercises</p>
          <p className="text-sm font-ui text-foreground">{breathingUsed} completed</p>
        </div>
      )}
    </div>
  );
