...
export default function EmotionalReplay({ sessionId }: Props) {
  ...
  if (!sessionId) {
    return <div className="text-xs text-muted-foreground italic text-center py-8">Select a moment to replay.</div>;
  }
  if (loading) {
    return <div className="text-xs text-muted-foreground text-center py-8">Loading replay…</div>;
  }
  if (!points.length) {
    return <div className="text-xs text-muted-foreground italic text-center py-8">No messages to replay.</div>;
  }
      ...
      {moments.length > 0 && (
        <div>
          <p className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground mb-2 flex items-center gap-1.5">
            <Sparkles className="w-3 h-3" /> Key moments
          </p>
          <div className="space-y-1.5 max-h-32 overflow-y-auto pr-1">
            {moments.map((m) => (
              <button key={`${m.position}-${m.type}`} onClick={() => setIdx(m.position)}
                className="w-full text-left text-[11px] px-2.5 py-1.5 rounded-lg border border-border/20 hover:border-primary/30 transition-colors flex items-center gap-2">
