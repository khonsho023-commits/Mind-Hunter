...
import { ArrowRight, Share2, Calendar, Clock, Activity, MessageSquare } from 'lucide-react';
    ...
    <div className="glass-strong rounded-2xl p-5 flex flex-col gap-4 h-full min-h-[520px]">
      <div>
        ...
        <div className="mt-2 flex items-center gap-3 text-sm font-ui text-foreground">
          ...
          {date ? date.toLocaleDateString(undefined, { weekday: 'long', month: 'short', day: 'numeric', year: 'numeric' }) : 'All time'}
        </div>
        {active && (
          <div className="mt-1 flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>·</span>
            <span className="capitalize">{active.summary_emotion ?? 'reflection'}</span>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 min-h-0 flex flex-col gap-3">
        <div className="space-y-1.5 max-h-32 overflow-y-auto pr-1">
          {sessions.length === 0 ? (
            <p className="text-xs text-muted-foreground italic">No conversations at this moment.</p>
          ) : sessions.map(s => (
            <button
              key={s.id}
              onClick={() => setActiveId(s.id)}
              className={`w-full text-left text-xs px-3 py-2 rounded-lg transition-all border ${
                s.id === activeId
            ...
            >
              ...
              <span className="capitalize">{s.summary_emotion ?? 'session'}</span>
            </button>
          ))}
        </div>
              ...
              <AnimatePresence mode="popLayout">
                {messages.length === 0 ? (
                  <motion.p key="empty"
                    ...
                    {active ? 'No messages yet.' : 'Select a moment to preview.'}
                  </motion.p>
                  ...
                  <motion.div key={m.id}
                    ...
                    <div className={`max-w-[85%] rounded-lg px-3 py-1.5 text-[11px] leading-snug font-body ${
                      m.role === 'user'
                        ...
                        : 'glass border-l-2 border-l-gold/50 text-foreground/90'
                    }`}>
                      {m.content}
                    </div>
                  </motion.div>
      ...
      <div className="grid grid-cols-2 gap-2 pt-1">
        ...
        >
          <Share2 className="w-3 h-3" /> Share Moment
        </button>
      </div>
    </div>
  );
