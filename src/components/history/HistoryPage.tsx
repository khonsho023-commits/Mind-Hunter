...
export default function HistoryPage() {
  ...
  return (
    <PageShell title="History Vault" subtitle="Navigate time to explore your conversations.">
      <div className="grid grid-cols-1 lg:grid-cols-[18rem_1fr_22rem] gap-6 min-h-[640px]">
        ...
        <div className="relative flex flex-col items-center justify-start gap-6 py-4">
          <p className="text-[10px] font-ui tracking-[0.35em] uppercase text-muted-foreground">
            Drag · Scroll · Click to navigate time
          </p>
          ...
          <div className="flex flex-col items-center gap-2">
            <div className="text-base font-display gold-text tracking-wider">
              {(filterDate ?? new Date()).toLocaleDateString(undefined, {
                weekday: 'long', month: 'long', day: 'numeric', year: 'numeric',
              })}
            </div>
            <div className="flex items-center gap-3">
              <span className="text-xs text-muted-foreground tabular-nums">
                {matched.length} {matched.length === 1 ? 'conversation' : 'conversations'}
              </span>
              {filterDate && (
                <button
      ...
      {loading && (
        <p className="text-xs text-muted-foreground mt-6 text-center">Loading memories...</p>
      )}
    </PageShell>
  );
}
