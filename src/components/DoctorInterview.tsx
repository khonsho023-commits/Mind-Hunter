...
const INTRO_MESSAGE = "Hello, I'm Dr. Sentinel. I'm glad you've decided to take this step. Before we begin our sessions together, I'd like to get to know you a little better. There's no rush — just answer in whatever way feels right.";

const FIRST_QUESTION = "Let's start gently. How have you been feeling emotionally over the past few weeks?";
    ...
    await streamChat({
      ...
      onError: (msg) => {
        ...
        setBubbles(prev => [...prev, {
          ...
          role: 'therapist',
          content: "I appreciate you sharing that. Could you tell me a bit more about what's been on your mind lately?",
        }]);
      },
    });
  }, [input, isTyping, isComplete, exchangeCount, chatHistory, user, updateProfile, setStage]);
      ...
      <div className="flex-1 flex flex-col min-w-0 relative">
                  ...
                  <div className="max-w-[80%]">
                    {bubble.role === 'therapist' && (
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        <span className="text-xs font-ui text-muted-foreground">Dr. Sentinel</span>
                      </div>
                    )}
                    <div className={`rounded-2xl px-5 py-3.5 ${
                      bubble.role === 'user'
                        ? 'bg-primary/15 border border-primary/25 text-foreground'
                        : 'glass border-l-2 border-l-primary/40 text-foreground'
                    }`}>
        ...
        {isComplete && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 text-center border-t border-border/30">
            <p className="text-xs font-ui text-muted-foreground animate-pulse">Starting your session...</p>
          </motion.div>
        )}
      </div>
    </div>
