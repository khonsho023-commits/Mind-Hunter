...
export async function listMemories(userId: string, limit = 200): Promise<EmotionalMemory[]> {
  const { data, error } = await supabase
    .from('emotional_memories' as never)
    .select('*')
    .eq('user_id', userId)
...
export async function listRelationships(userId: string): Promise<MemoryRelationship[]> {
  const { data, error } = await supabase
    .from('memory_relationships' as never)
    .select('*')
    .eq('user_id', userId);
...
async function logEvent(
  ...
  await sbExt.from('memory_events').insert({
    user_id: userId,
    memory_id: memoryId,
...
export async function reconcileRelationships(userId: string) {
  ...
  if (inserts.length) {
    await sbExt.from('memory_relationships').insert(inserts);
  }
}
