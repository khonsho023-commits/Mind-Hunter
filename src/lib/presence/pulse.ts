...
export async function loadRecentPulses(userId: string, days = 14): Promise<DailyPulse[]> {
  ...
  const { data, error } = await supabase
    .from('emotional_pulses' as never)
    .select('*')
    .eq('user_id', userId)
...
export async function recomputeTodayPulse(userId: string) {
  ...
  const { data: sessions } = await supabase
    .from('sessions')
    .select('id, dominant_emotion, summary_emotion, summary_intensity, message_count')
    .eq('user_id', userId)
  ...
  let intensitySum = 0, intensityN = 0, messageSum = 0;
  for (const s of list) {
    const e = (s as { dominant_emotion?: string; summary_emotion?: string }).dominant_emotion
           ?? (s as { summary_emotion?: string }).summary_emotion;
    ...
    messageSum += (s as { message_count?: number }).message_count ?? 0;
  }
  ...
  await sbExt.from('emotional_pulses').upsert(row, { onConflict: 'user_id,pulse_date' });
  return row;
}
